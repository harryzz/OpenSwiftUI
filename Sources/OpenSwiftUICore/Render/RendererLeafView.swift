...
extension RendererLeafView {
    ...
    package static func makeLeafView(view: _GraphValue<Self>, inputs: _ViewInputs) -> _ViewOutputs {
        ...
        // [wandr] Note: geometric gesture hit-testing uses each GestureResponder's OWN layout frame
        // (Approach A, see GestureViewModifier.GestureFilter), so leaf views do NOT need to emit
        // per-view responders here. (If non-gesture leaf hit-testing is ever needed, emit a
        // geometry-carrying ViewResponder under `requiresViewResponders`.)
        return outputs
    }
}
